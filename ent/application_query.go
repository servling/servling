...
func (aq *ApplicationQuery) QueryServices() *ServiceQuery {
...
func (aq *ApplicationQuery) QueryTemplate() *TemplateQuery {
...
func (aq *ApplicationQuery) WithServices(opts ...func(*ServiceQuery)) *ApplicationQuery {
	query := (&ServiceClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) WithTemplate(opts ...func(*TemplateQuery)) *ApplicationQuery {
	query := (&TemplateClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := aq.withServices; query != nil {
		if err := aq.loadServices(ctx, query, nodes,
			func(n *Application) { n.Edges.Services = []*Service{} },
			func(n *Application, e *Service) { n.Edges.Services = append(n.Edges.Services, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTemplate; query != nil {
		if err := aq.loadTemplate(ctx, query, nodes, nil,
			func(n *Application, e *Template) { n.Edges.Template = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ApplicationQuery) loadServices(ctx context.Context, query *ServiceQuery, nodes []*Application, init func(*Application), assign func(*Application, *Service)) error {
	...
	query.Where(predicate.Service(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.ServicesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.application_services
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_services" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "application_services" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *ApplicationQuery) loadTemplate(ctx context.Context, query *TemplateQuery, nodes []*Application, init func(*Application), assign func(*Application, *Template)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Application)
	for i := range nodes {
		if nodes[i].template_applications == nil {
			continue
		}
		fk := *nodes[i].template_applications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "template_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
