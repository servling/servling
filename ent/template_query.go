...
func (tq *TemplateQuery) QueryApplications() *ApplicationQuery {
...
func (tq *TemplateQuery) WithApplications(opts ...func(*ApplicationQuery)) *TemplateQuery {
	query := (&ApplicationClient{config: tq.config}).Query()
...
func (tq *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if query := tq.withApplications; query != nil {
		if err := tq.loadApplications(ctx, query, nodes,
			func(n *Template) { n.Edges.Applications = []*Application{} },
			func(n *Template, e *Application) { n.Edges.Applications = append(n.Edges.Applications, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TemplateQuery) loadApplications(ctx context.Context, query *ApplicationQuery, nodes []*Template, init func(*Template), assign func(*Template, *Application)) error {
	...
	query.Where(predicate.Application(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(template.ApplicationsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.template_applications
		if fk == nil {
			return fmt.Errorf(`foreign-key "template_applications" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "template_applications" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
