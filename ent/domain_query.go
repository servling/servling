...
func (dq *DomainQuery) QueryIngresses() *IngressQuery {
	query := (&IngressClient{config: dq.config}).Query()
...
func (dq *DomainQuery) Clone() *DomainQuery {
	...
	return &DomainQuery{
		...
		withIngresses: dq.withIngresses.Clone(),
...
func (dq *DomainQuery) WithIngresses(opts ...func(*IngressQuery)) *DomainQuery {
	query := (&IngressClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withIngresses = query
	return dq
}
...
func (dq *DomainQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Domain, error) {
	...
	if query := dq.withIngresses; query != nil {
		if err := dq.loadIngresses(ctx, query, nodes,
			func(n *Domain) { n.Edges.Ingresses = []*Ingress{} },
			func(n *Domain, e *Ingress) { n.Edges.Ingresses = append(n.Edges.Ingresses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DomainQuery) loadIngresses(ctx context.Context, query *IngressQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *Ingress)) error {
	...
	query.Where(predicate.Ingress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(domain.IngressesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.domain_ingresses
		if fk == nil {
			return fmt.Errorf(`foreign-key "domain_ingresses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "domain_ingresses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
