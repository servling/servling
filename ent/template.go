...
func (e TemplateEdges) ApplicationsOrErr() ([]*Application, error) {
	if e.loadedTypes[0] {
...
func (*Template) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case template.FieldID, template.FieldName:
...
// QueryApplications queries the "applications" edge of the Template entity.
func (t *Template) QueryApplications() *ApplicationQuery {
	return NewTemplateClient(t.config).QueryApplications(t)
}
