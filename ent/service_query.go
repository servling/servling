...
func (sq *ServiceQuery) QueryApplication() *ApplicationQuery {
...
func (sq *ServiceQuery) WithApplication(opts ...func(*ApplicationQuery)) *ServiceQuery {
	query := (&ApplicationClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*Service, init func(*Service), assign func(*Service, *Application)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Service)
	for i := range nodes {
		if nodes[i].application_services == nil {
			continue
		}
		fk := *nodes[i].application_services
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "application_services" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
